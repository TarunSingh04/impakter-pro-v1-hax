...
const Learnings = () => {
  ...
  const [elearningActivated, setElearningActivated] = useState(false);
  ...
  const suscribe = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URI}/api/user/feature/e_learning?value=true`,
        {
          method: "PATCH",
          headers: {
  ...
  const fetchUserData = async () => {
    try {
      ...
      if (response.ok && data) {
        setElearningActivated(data?.e_learning); // Default to false if undefined
        setName(data.name);
        if (data?.e_learning) {
          setsubscribed(true);
        }
